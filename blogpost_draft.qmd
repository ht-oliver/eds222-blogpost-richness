...
This gives us meaningful richness values that we can use in our 
...
Our coefficients are very similar, which shouldn't be very surprising because we built our simulation on the results from our original model. But it does introduce a degree of randomness which allows us assess the accuracy of our model. Now, we can use simulated data to generate confidence intervals for our results.  We'll run our simulation 500 times

### **8) Simulating new datasets**
...
```{r}
# THIRD: Calculate confidence intervals
